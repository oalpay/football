...
public class ContactHelper implements ContactFilter, ContactListener {
	...
	public void warnEnemies(Fixture groundFixture, ENEMY_STATE state) {
		...
		for (int i = 0; i < contactList.size; i++) {
			...
			if (contact.isTouching()) {
				if (groundFixture.equals(contact.getFixtureA())
						&& dataB.equals("enemy")
						|| groundFixture.equals(contact.getFixtureB())
						&& dataA.equals("enemy")) {

					Enemy e = (Enemy) contact.getFixtureB().getBody()
							.getUserData();

					if (dataA.equals("ground"))
						e = (Enemy) contact.getFixtureB().getBody()
								.getUserData();
	...
	public boolean shouldCollide(Fixture fixtureA, Fixture fixtureB) {
		/*
		 * if(fixtureA.getBody().getUserData() instanceof Enemy){ Enemy e =
		 * (Enemy)fixtureA.getBody().getUserData(); if(e.isFlaggedForDelete())
		 * return false; }
		 * 
		 * if(fixtureB.getBody().getUserData() instanceof Enemy){ Enemy e =
		 * (Enemy)fixtureB.getBody().getUserData(); if(e.isFlaggedForDelete())
		 * return false; }
		 */
	...
	public void beginContact(Contact contact) {
		...
		if (checkIfContact(dataA, dataB, "ball", "enemy")) {
			...
			if (dataB.equals("enemy")) {
				enemyHit = (Enemy) contact.getFixtureB().getBody()
						.getUserData();

			} else {
				enemyHit = (Enemy) contact.getFixtureA().getBody()
						.getUserData();
		...
		if (checkIfContact(dataA, dataB, "ground", "enemy")) {
			...
			if (dataB.equals("enemy")) {
				...
				((ChainShape) contact.getFixtureA().getShape())
						.getVertex(0, v1);
				((ChainShape) contact.getFixtureA().getShape())
						.getVertex(1, v2);
	...
	public void endContact(Contact contact) {
		...
		/*
		 * if((dataA.equals("ball") && dataB.equals("player"))||
		 * (dataB.equals("ball") && dataA.equals("player"))){
		 * //if(player.isHoldState())
		 * ballBody.setLinearVelocity(ballBody.getLinearVelocity().x/10,
	...
	// CHECK CONTACT
	public boolean checkIfContact(String A, String B, String targetA,
			String targetB) {
		if (targetB.equals("ALL") && (A.equals(targetA) || B.equals(targetA)))
			return true;

		if ((A.equals(targetA) && B.equals(targetB))
				|| (B.equals(targetA) && A.equals(targetB)))
			return true;
	...
	public boolean checkIfContact(String A, String B, String targetA,
			String targetB, String exception) {
