...
public class HUD {
	...
	public void initTouchPad() {
		...
		touchpad.setBounds(stage.getWidth() - touchPadSize - touchPadGap,
				touchPadGap, touchPadSize, touchPadSize);
	...
	public void initButtons() {
		...
		btnPlay.setPosition(stage.getWidth() - btnSize * 1.5f,
				stage.getHeight() - btnSize * 1.5f);
		...
		btnPlay.addListener(new InputListener() {
			...
			public void touchUp(InputEvent event, float x, float y,
					int pointer, int button) {
				if (GameController.isGameRunning())
					GameController.setGameState(GameState.GAME_PAUSED);
				else
					GameController.setGameState(GameState.GAME_RUNNING);
			}
		});
	}
	...
	private void checkMovementControllerInput(float x, float y) {
		if (x >= 0 && y >= 0 && x <= controllerWidth && y <= controllerHeight) {
			if (x < controllerWidth / 3) { // GO TO LEFT
				player.keepMovingLeft();
			} else if (x > controllerWidth * 2 / 3) { // GO TO RIGHT
				player.keepMovingRight();
			} else{
				player.stopMovingHorizontal();
			}
	...
	public void showGameOver() {
		...
		if (isGameRunning) {
			...
			endScreenTable.setPosition(
					stage.getWidth() / 2 - endScreenTable.getWidth() / 2,
					stage.getHeight() / 2 - endScreenTable.getHeight() / 2);

			endScreenTable.add(new Label("GAME OVER!", skin, "ending"))
					.colspan(2).padBottom(btnSize / 2).row();
